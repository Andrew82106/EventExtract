...
class SchemaEvaluator:
    ...
    def evaluate_entities(self, predicted: List[Dict], ground_truth: List[Dict], logger=None) -> Dict:
        """评估实体识别结果（使用词相似度匹配）"""
        ...
        for pred_entity in predicted:
            ...
            if logger:
                logger.printLog(f"Predicted entity: {pred_entity}")
                logger.printLog(f"Best match: {best_match}")
                logger.printLog(f"Similarity: {similarity}")
                logger.printLog(f"alternative matches:{ground_truth}")
